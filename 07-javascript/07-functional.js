...
const mapNameFamily = (characters) => {
  ...
  characters.forEach((element) => {
    output.push(element.name + ", of House " + element.house);
  });
  return output;
};
...
const reduceHouses = (characters) => {
  ...
  // initialize numCharacters object
  let numCharacters = {};
  houses.forEach((house) => {
    numCharacters[house] = 0;
  });

  characters.forEach((character) => {
    if (numCharacters[character.house] != "undefined") {
      numCharacters[character.house] += 1;
    } else {
      // if a new house is found add it to numCharacters and init at 1
      numCharacters[character.house] = 1;
    }
  });
